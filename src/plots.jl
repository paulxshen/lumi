...
function plotfield(args...; axis=(;), rulers, kwargs...)
    fig = args[1]
    u = args[end]
    N = ndims(u)
    if N == 2
...
function plotsim(prob, sol, field_names=[:Hz], geometry_names=[:ϵ]; path=nothing)
    ...
    for (i, k) = enumerate(field_names)
        a = sol.u(k)
        a /= maximum(abs, a)
        plotfield(fig[i, 1], a; rulers=rulers[k], colormap=:seismic, colorrange=(-1, 1), axis=(; title="$k"))
    end
    for (i, k) = enumerate(geometry_names)
        i += length(field_names)
        a = prob.geometry(k)[1]
        a /= maximum(abs, a)
        plotfield(fig[i, 1], a; rulers=rulers.default, colormap=:grays, colorrange=(0, 1), axis=(; title="$k"))
    end
